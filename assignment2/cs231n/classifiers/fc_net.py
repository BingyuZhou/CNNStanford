...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        num_hid = len(hidden_dims)
        self.params['W1'] = np.random.normal(0, weight_scale, (input_dim, hidden_dims[0]))
        self.params['b1'] = np.zeros((hidden_dims[0],))
        if (use_batchnorm):
            self.params['gamma1'] = np.ones((hidden_dims[0],))
            self.params['beta1'] = np.ones((hidden_dims[0],))
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if self.use_batchnorm:
                out_BN, cache  = batchnorm_forward(out, self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)], self.bn_params[i])
                cache_all.append(cache)
                out_relu, cache = relu_forward(out_BN)
                cache_all.append(cache)
            else:
                out_relu, cache = relu_forward(out)
                cache_all.append(cache)
            if self.use_dropout:
                out_dropout, cache = dropout_forward(out_relu, self.dropout_param)
                cache_all.append(cache)
                input_data = out_dropout
            else:
                input_data = out_relu
        scores, cache = affine_forward(input_data, self.params['W'+str(i+2)], self.params['b'+str(i+2)])
        cache_all.append(cache)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
